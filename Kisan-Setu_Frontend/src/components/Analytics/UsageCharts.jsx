...
const Analytics = () => {
  ...
  // Randomize water data on every mount/refresh – ONLY IN SIMULATION
  React.useEffect(() => {
    if (isLive) {
      setWaterAnalytics([]);
      setAvgUsage(0);
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-5 duration-700">
      ...
      <div className="ring-0 min-h-[500px]">
        {isLive ? (
          <div className="flex flex-col items-center justify-center p-20 bg-white/50 dark:bg-slate-900/50 rounded-[4rem] border-2 border-dashed border-slate-200 dark:border-slate-800 text-center space-y-6">
            <div className="p-6 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-400">
               <Calendar className="w-12 h-12" />
            </div>
            <div>
               <h3 className="text-2xl font-black text-slate-800 dark:text-white uppercase tracking-tighter">Awaiting DB Synchronization</h3>
               <p className="text-slate-400 dark:text-slate-500 font-bold text-xs uppercase tracking-widest mt-2">Historical analytics are currently locked for Live Telemetry sessions.</p>
            </div>
          </div>
        ) : (
